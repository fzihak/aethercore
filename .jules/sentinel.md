...
## 2025-06-25 - Fix JWT Signature Bypass
**Vulnerability:** The JWT verifier logic optionally skipped signature verification if the provided public key was `nil`.
**Learning:** Always enforce cryptographic signature verification for tokens, never make it optional based on the presence of the validation key, which can result in accepting forged tokens as valid.
**Prevention:** Ensure that initialization of token verifiers explicitly requires the validation keys and fails verification if the key is missing.
