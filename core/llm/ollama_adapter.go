...
func toOllamaToolCalls(toolCalls []ToolCall) []ollamaToolCall {
	...
	ollamaCalls := make([]ollamaToolCall, 0)
...
//
//nolint:gocritic // m is small enough for this short-lived call
	...
	default:
		...
		if m.Role == "tool" && len(m.ToolResults) > 0 {
			...
			if len(m.ToolResults) == 1 {
				...
				raw, _ := json.Marshal(m.ToolResults) //nolint:errchkjson,musttag // tool results are safely structured
				content = string(raw)
			}
		}
...
//
//nolint:gocritic // r is small enough for this short-lived call
...
//
//nolint:gocritic // t is small enough for this short-lived call
